...
export function createApplicationsRouter(pool: Pool) {
  ...
  router.post('/auto', requireAuth, async (req: AuthRequest, res: Response) => {
    ...
    if (!job?.id || !candidate?.id) return res.status(400).json({ success: false, message: 'job.id and candidate.id are required.' });
    try {
      ...
      return res.status(500).json({ success: false, message: 'Automation error.' });
    }
  });
  ...
  router.post('/', requireAuth, async (req: AuthRequest, res: Response) => {
    ...
    if (!jobId || !candidateId) return res.status(400).json({ success: false, message: 'jobId and candidateId are required.' });
    try {
      await pool.query(
      ...
      return res.json({ success: true, message: 'Application recorded.' });
    } catch (error: unknown) {
      const dbErr = error as { code?: string };
      if (dbErr.code === '23503') return res.status(404).json({ success: false, message: 'Job or candidate not found.' });
      console.error('[Apply POST Error]:', error);
      return res.status(500).json({ success: false, message: 'Error recording application.' });
    }
  });
  ...
  router.patch('/status', requireAuth, async (req: AuthRequest, res: Response) => {
    ...
    if (!jobId || !candidateId || !status) return res.status(400).json({ success: false, message: 'Missing fields.' });
  ...
  router.get('/status', requireAuth, async (req: AuthRequest, res: Response) => {
    ...
    if (!jobId || !candidateId) return res.status(400).json({ success: false, message: 'Missing jobId or candidateId.' });
    try {
      const result = await pool.query(
      ...
      return res.status(500).json({ success: false, message: 'Error querying application.' });
    }
  });
